...
import { readableStreamToString, InducedAiError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new InducedAiError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new InducedAiError(e, e.response?.data)
                }
                if (innerError instanceof InducedAiError) {
                    // Got "InducedAiError" from the above try block
                    throw innerError;
                }
